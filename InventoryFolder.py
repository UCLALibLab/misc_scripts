...
def writeReports(data, reportsPath, targetName):
  ...
  for fileInfo in sortedCats:
    fileType = fileInfo[0]
    summaryFile.write(fileType + ": " + "{:,}".format(data['cats'][fileType]['count']) + " (" + humanize.naturalsize(data['cats'][fileType]['size'], gnu=True) + ")\n")
  ...
  for folder in data['dirs']:
    ...
    for fileInfo in sortedCats:
      fileType = fileInfo[0]
      summaryFile.write(fileType + ": " + "{:,}".format(data['dirs'][folder]['cats'][fileType]['count']) + " (" + humanize.naturalsize(data['dirs'][folder]['cats'][fileType]['size'], gnu=True) + ")\n")
    ...
    for fileInfo in sortedCats:
      fileType = fileInfo[0]
      folderFile.write(fileType + ": " + "{:,}".format(data['dirs'][folder]['cats'][fileType]['count']) + " (" + humanize.naturalsize(data['dirs'][folder]['cats'][fileType]['size'], gnu=True) + ")\n")
...
def walkPath(targetPath):
  ...
  localStats = {'count': 0, 'size': 0, 'types': {}, 'cats': {}}
  ...
  for obj in os.listdir(targetPath):
    ...
    if (not os.path.isdir(objPath)):
      ...
      if (fileType not in data['types']):
        data['types'][fileType] = {'count': 1, 'size': fileSize}
      else:
        data['types'][fileType]['count'] += 1
        data['types'][fileType]['size'] += fileSize
      if (fileType not in data['cats']):
        data['cats'][fileType] = {'count': 1, 'size': fileSize}
      else:
        data['cats'][fileType]['count'] += 1
        data['cats'][fileType]['size'] += fileSize
      ...
      if (fileType not in data['dirs']['root']['types']):
        data['dirs']['root']['types'][fileType] = {'count': 1, 'size': fileSize}
      else:
        data['dirs']['root']['types'][fileType]['count'] += 1
        data['dirs']['root']['types'][fileType]['size'] += fileSize
      if (fileType not in data['dirs']['root']['cats']):
        data['dirs']['root']['cats'][fileType] = {'count': 1, 'size': fileSize}
      else:
        data['dirs']['root']['cats'][fileType]['count'] += 1
        data['dirs']['root']['cats'][fileType]['size'] += fileSize
...
for obj in os.listdir(targetPath):
  ...
  for fileType in results['types']:
    ...
    if (fileType not in data['types']):
      data['types'][fileType] = {'count': typeCount, 'size': typeSize}
    else:
    ...
    if (fileType not in data['dirs'][thisPath]['types']):
      data['dirs'][thisPath]['types'][fileType] = {'count': typeCount, 'size': typeSize}
    else:
  ...
  for fileCat in results['cats']:
    ...
    if (fileCat not in data['cats']):
      data['cats'][fileCat] = {'count': catCount, 'size': catSize}
    else:
    ...
    if (fileCat not in data['dirs'][thisPath]['cats']):
      data['dirs'][thisPath]['cats'][fileCat] = {'count': catCount, 'size': catSize}
    else:
      data['dirs'][thisPath]['cats'][fileCat]['count'] += catCount
      data['dirs'][thisPath]['cats'][fileType]['size'] += catSize
